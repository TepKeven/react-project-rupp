...
function AddressEditComponent({address = {}, addressIndex = 0, countries = []}){
            ...
            <div class="mb-4">
                ...
                <select id={`address_country_${addressIndex}`} name={`address_description[${addressIndex}][country_id]`} class="form-control">
                    {countries.map(country => (
                        <option value={country.country_id} selected={address.country_id == country.country_id ? "selected" : ""}>{country.name}</option>
                    ))}
                </select>
            </div>
        </div>
